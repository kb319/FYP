...
global N;
N = 20;
x_plus = [initial_indoor_temp, initial_flow_temp]';
...
for i = 1+day_of_year:n+day_of_year
   ...
   Beq = vertcat(x_plus, yend, Beq_base, Beq_temps);
   ...
   x_plus = A*x1_o+B*[outdoor_temp(i) Q_HP_new]';
   ...
   T_in_plot = [T_in_plot x_plus(1)]; % y-target as the cost is based around it, so this gives back real indoor temp
   T_flow_plot = [T_flow_plot x_plus(2)];
...
yyaxis right;
ylabel("Power (kW)");
plot(1:size(Q_out,2),Q_out,'LineWidth',linesize);
yline(0,'--','color','g');
yline(6,'--','color','g');
ylim([0 50])
...
function objective = cost_function(d)
   ...
   objective = sum((d(1:x_size:N*2-1)-target_indoor_temp).^2)+d(psi_vec(1));
